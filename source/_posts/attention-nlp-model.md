...
The attention mechanism is a sublayer in the network and a full layer is `LayerNorm(x + Sublayer(x))`. A transformer encoder uses the full attention layer result and passes the result to a feed-forward layer with residual connection and normalisation.
...
Where Q,K,V devote query, key, and value in an attention operation. $Attention(\cdot)$ is a standard self-attention and the 2 attention stream shares a same set of parameters. For initialisation, $g\_i^{(0)}$ is initialised with a trainable vector and $h\_i^{(0)}$ is the corresponding word embedding for input position $i$.
